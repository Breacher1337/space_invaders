...
def main(player, player_stats, enemy_vel, wave_length):
	game_music()
...
def select_difficulty(selected_ship_name):
	def set_difficulty(difficulty):
		sound_onclick()
		if difficulty == "Easy":
			enemy_vel = 0.5  # Increased enemy speed for Easy
			wave_length = 6   # More enemies on Easy
		elif difficulty == "Medium":
			enemy_vel = 0.7  # Increased enemy speed for Medium
			wave_length = 8   # More enemies on Medium
		elif difficulty == "Hard":
			enemy_vel = 1.0  # Even faster enemies on Hard
			wave_length = 10  # More enemies on Hard
		
		# Start the game with selected enemy settings
		start_game(selected_ship_name, enemy_vel, wave_length)

	difficulty_menu = pygame_menu.Menu("Select Difficulty", WIDTH, HEIGHT, theme=pygame_menu.themes.THEME_DARK)
	difficulty_menu.add.button("Easy", lambda: set_difficulty("Easy"))
	difficulty_menu.add.button("Medium", lambda: set_difficulty("Medium"))
	difficulty_menu.add.button("Hard", lambda: set_difficulty("Hard"))
	difficulty_menu.add.button("Back", main_menu)
	difficulty_menu.mainloop(WIN)
...
def open_character_select():
	...
	settings.save_settings()
	selected_ship_name = character_select(WIN)
	select_difficulty(selected_ship_name)
